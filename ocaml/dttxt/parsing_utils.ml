...
let get_feature_index name parsed_features =
  let rec aux name parsed_features acc =
    match parsed_features with
...
let rec unname_tree dt parsed_features =
  match dt with
  | [] -> []
...
let get_feature_name_at_index index parsed_features =
  let rec aux index parsed_features cpt =
    match parsed_features with
...
let rec _string_of_string_list (l : string list) : string =
  match l with
  | [ ] -> ""
  | [t] -> "\"" ^ t ^ "\""
...
let string_of_vector_element ve =
  match ve with
...
let string_of_tree_element ve =
  match ve with
  | ParsedLeaf c -> "L(" ^ string_of_int c ^ ")"
  | ParsedNode (index_feature, value) -> "N(" ^ string_of_int index_feature ^ ", " ^ string_of_value value ^ ")"

let rec _string_of_tree t =
  match t with
  | [] -> ""
